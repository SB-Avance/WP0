...
def SidebarView(on_nav, on_logout=None, user_name=None, user_rol=None):
    ...
    destinations.append(
        ft.NavigationRailDestination(
            icon_content=ft.Icon(ft.icons.CHAT, tooltip="Ver conversaciones de WhatsApp"),
            label="Chats"
        )
    )
    ...
    if user_rol == 'administrador':
        destinations.extend([
            ft.NavigationRailDestination(
                icon_content=ft.Icon(ft.icons.SMART_TOY, tooltip="Gestionar chatbots (CRUD)"),
                label="Chatbots"
            ),
            ft.NavigationRailDestination(
                icon_content=ft.Icon(ft.icons.PEOPLE, tooltip="Gestionar usuarios (CRUD)"),
                label="Usuarios"
            ),
            ft.NavigationRailDestination(
                icon_content=ft.Icon(ft.icons.ARTICLE, tooltip="Gestionar templates (CRUD)"),
                label="Templates"
            ),
            ft.NavigationRailDestination(
                icon_content=ft.Icon(ft.icons.PHONE, tooltip="Gestionar cuentas WhatsApp (CRUD)"),
                label="Cuentas WA"
            ),
        ])
