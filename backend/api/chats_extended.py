...
def get_chats_por_grupo(grupo_id):
    ...
    try:
        ...
        if expand_contacto:
            expand_parts.append("cr321_contactorelacion($select=cr321_contactoid,cr321_fromnombre,cr321_telefono)")
        ...
        if response.status_code == 200:
            ...
            for item in data.get('value', []):
                ...
                chat = {
                    ...
                    'contacto_nombre': contacto.get('cr321_fromname') if contacto else None
                }
                chats.append(chat)
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                ...
                'resumen': {
                    'entrantes': entrantes,
                    'salientes': salientes,
                    'primera_interaccion': historial[0]['fecha_hora'] if historial else None,
                    'ultima_interaccion': historial[-1]['fecha_hora'] if historial else None
                },
                'historial': historial
            }), 200
