...
def get_access_token():
    ...
    if "access_token" in result:
        ...
        raise Exception(f"Error obteniendo token: {result.get('error_description', 'Unknown')}")
...
def make_odata_request(entity_set, params=None):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            logger.info(f"[✅ DASHBOARD] {entity_set} - {data.get('@odata.count', len(data.get('value', [])))} registros")
...
def metricas_grupos():
    ...
    try:
        ...
        success_grupos, data_grupos, error_grupos = make_odata_request('cr321_grups', params_grupos)
        ...
        for grupo in grupos:
            ...
            success_msgs, data_msgs, _ = make_odata_request('cr321_adatawp0s', params_mensajes)
            
            if success_msgs:
                total_mensajes = data_msgs.get('@odata.count', 0)
                mensajes = data_msgs.get('value', [])
                ultimo_mensaje = mensajes[0].get('cr321_createdon') if mensajes else None
                ...
                success_ent, data_ent, _ = make_odata_request('cr321_adatawp0s', params_entrantes)
                success_sal, data_sal, _ = make_odata_request('cr321_adatawp0s', params_salientes)
                ...
                metricas_por_grupo.append({
                    ...
                    "tasa_respuesta": f"{(mensajes_salientes / mensajes_entrantes * 100):.1f}%" if mensajes_entrantes > 0 else "N/A"
                })
            else:
                # Grupo sin mensajes
                metricas_por_grupo.append({
...
def metricas_generales():
    ...
    try:
        ...
        success_total, data_total, _ = make_odata_request('cr321_adatawp0s', params_total)
        ...
        success_cont, data_cont, _ = make_odata_request('cr321_contactos', params_contactos)
        ...
        return jsonify({
            ...
            "metricas": {
                ...
                "mensajes_por_contacto": round(total_mensajes / total_contactos, 2) if total_contactos > 0 else 0
            },
            "periodo": {
                "desde": fecha_desde or "inicio",
                "hasta": fecha_hasta or "ahora"
...
def volumetria():
    ...
    try:
        ...
        filter_string = f"cr321_createdon ge {fecha_desde} and cr321_createdon le {fecha_hasta}"
        ...
        if not success:
            return jsonify({
                "success": False,
                "error": f"Error obteniendo datos: {error}"
            }), 500
        ...
        resumen = {
            ...
            "dia_max": {
                "fecha": dia_max['periodo'],
                "total": dia_max['total']
            } if dia_max else None,
            "dia_min": {
                "fecha": dia_min['periodo'],
                "total": dia_min['total']
            } if dia_min else None
        }
...
def tendencias():
    ...
    try:
        ...
        for mensaje in mensajes:
            ...
            mensajes_por_dia_semana[dia_semana] = mensajes_por_dia_semana.get(dia_semana, 0) + 1
        
        # Calcular tendencia
        dias_ordenados = sorted(mensajes_por_dia.items())
        if len(dias_ordenados) >= 2:
            primera_mitad = sum(v for k, v in dias_ordenados[:len(dias_ordenados)//2])
            segunda_mitad = sum(v for k, v in dias_ordenados[len(dias_ordenados)//2:])
        ...
        # Hora pico
        hora_pico = max(mensajes_por_hora.items(), key=lambda x: x[1])[0] if mensajes_por_hora else "N/A"
        
        # Día de semana pico
        dia_semana_pico = max(mensajes_por_dia_semana.items(), key=lambda x: x[1])[0] if mensajes_por_dia_semana else "N/A"
        ...
        return jsonify({
            "success": True,
            "tendencias": {
                "mensajes_por_dia": [{"fecha": k, "total": v} for k, v in dias_ordenados],
                "crecimiento_periodo": crecimiento_str,
                "hora_pico": hora_pico,
                "dia_semana_pico": dia_semana_pico,
                "total_periodo": len(mensajes),
                "promedio_diario": round(len(mensajes) / dias, 2) if dias > 0 else 0
            },
            "periodo": {
                "desde": fecha_inicio,
                "hasta": fecha_fin,
