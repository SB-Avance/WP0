...
def get_templates():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"error": f"Error al obtener templates: {response.status_code}"}), response.status_code
...
def create_template():
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            template_id = location.split("(")[-1].split(")")[0] if "(" in location else None
            return jsonify({"success": True, "id": template_id, "message": "Template creado exitosamente"}), 201
        else:
            return jsonify({"error": f"Error al crear template: {response.status_code}", "details": response.text}), response.status_code
...
def update_template(template_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            return jsonify({"success": True, "message": "Template actualizado exitosamente"}), 200
        else:
            return jsonify({"error": f"Error al actualizar template: {response.status_code}", "details": response.text}), response.status_code
...
def delete_template(template_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            return jsonify({"success": True, "message": "Template eliminado exitosamente"}), 200
        else:
            return jsonify({"error": f"Error al eliminar template: {response.status_code}", "details": response.text}), response.status_code
