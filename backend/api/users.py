...
from goot import get_user_by_email, create_user, print_all_users, get_all_users, get_token, DATAVERSE_URL
...
def update_user(current_user, user_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            ...
            return jsonify({"error": f"Error al actualizar: {response.status_code}"}), response.status_code
...
def delete_user(current_user, user_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            ...
            return jsonify({"error": f"Error al eliminar: {response.status_code}"}), response.status_code
