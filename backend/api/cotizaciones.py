...
from back import get_token, DATAVERSE_URL
import requests

cotizaciones_bp = Blueprint('cotizaciones', __name__)

ENTITY_SET = "cr321_cotizacions"  # Dataverse pluraliza cr321_cotizacion -> cr321_cotizacions
...
def create_cotizacion():
    ...
    try:
        ...
        if 'cr321_contactoId' in data:
            cotizacion_data['cr321_contactoId@odata.bind'] = f"/cr321_contactos({data['cr321_contactoId']})"
        ...
        if response.status_code in [200, 201, 204]:
            if response.status_code == 204:
                ...
                created_id = response.headers.get('OData-EntityId', '').split('(')[-1].rstrip(')')
