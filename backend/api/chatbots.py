...
def get_chatbots():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({"error": f"Error al obtener chatbots: {response.status_code}"}), response.status_code
...
def create_chatbot():
    ...
    try:
        ...
        if response.status_code in [200, 201, 204]:
            return jsonify({"success": True, "message": "Chatbot creado exitosamente"}), 201
        else:
            return jsonify({"error": f"Error al crear chatbot: {response.status_code}"}), response.status_code
...
def update_chatbot(chatbot_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            return jsonify({"success": True, "message": "Chatbot actualizado exitosamente"}), 200
        else:
            return jsonify({"error": f"Error al actualizar chatbot: {response.status_code}"}), response.status_code
...
def delete_chatbot(chatbot_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            return jsonify({"success": True, "message": "Chatbot eliminado exitosamente"}), 200
        else:
            return jsonify({"error": f"Error al eliminar chatbot: {response.status_code}"}), response.status_code
