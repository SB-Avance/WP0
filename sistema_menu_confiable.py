...
class SistemaMenuConfiable:
    ...
    def obtener_opciones(self, forzar_refresh: bool = False) -> List[OpcionMenu]:
        ...
        """
        # Verificar cache
        if not forzar_refresh and self._cache and not self._cache.is_expired(MenuConfig.CACHE_DURATION_MINUTES):
    ...
    def _procesar_chatbots(self, chatbots: List[Dict]) -> List[OpcionMenu]:
        ...
        for chatbot in chatbots:
            ...
            if not nombre_categoria:
                self._log(f"⚠️  Chatbot {chatbot.get('cr321_chatbotid')} sin nombre, omitiendo")
                continue
            ...
            if len(elementos) > MenuConfig.MAX_OPCIONES_POR_CATEGORIA:
                self._log(f"⚠️  Categoría '{nombre_categoria}' tiene {len(elementos)} elementos, " +
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("   Asegúrate de que el archivo existe y tiene las credenciales correctas")
    except Exception as e:
        print(f"❌ Error: {e}")
