...
class SistemaMenuJSON:
    ...
    def _cargar_config_chatbot(self, nombre_chatbot: str) -> ConfigMenu:
        ...
        if response.status_code == 200:
            ...
            if not config_json:
                raise Exception(
                    f"Campo cr321_config vacío para chatbot: {nombre_chatbot}"
                )
    ...
    def _validar_grupos_menu(self):
        ...
        try:
            ...
            if response.status_code == 200:
                grupos_validos = {
                    g["cr321_grupoid"]: g.get("cr321_nombre", "Sin nombre")
        ...
        if grupos_invalidos:
            ...
            for item in grupos_invalidos[:3]:  # Mostrar primeros 3
                print(
                    f"     - {item['submenu']}: {item['nombre']} → {item['grupo_id']}"
                )
    ...
    def procesar_seleccion(self, from_user: str, numero_seleccion: str) -> str:
        ...
        grupo_final = self._determinar_grupo_final(
            handler_codigo, submenu.get("grupo_id")
        )
        ...
        try:
            ...
            return self.config_menu.configuracion_general.get(
                "mensaje_error", "Lo siento, hubo un error. Por favor intenta de nuevo."
            )

    def _determinar_grupo_final(
        self, codigo_handler: str, grupo_submenu: Optional[str]
    ) -> Optional[str]:
        """
    ...
    def _cargar_handler_dinamico(self, codigo: str):
        ...
        try:
            ...
            # Instanciar
            instancia = clase_handler(dataverse_url=DATAVERSE_URL, headers=self.headers)
