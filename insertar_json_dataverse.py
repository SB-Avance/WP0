...
def crear_chatbot(headers: dict, nombre: str, config_json: str) -> str:
    ...
    response = requests.post(
        f"{DATAVERSE_URL}/cr321_chatbots", headers=headers, json=data
    )
...
def actualizar_chatbot(headers: dict, guid: str, config_json: str):
    ...
    data = {"cr321_config": config_json}

    response = requests.patch(
        f"{DATAVERSE_URL}/cr321_chatbots({guid})", headers=headers, json=data
    )
...
def main():
    ...
    try:
        ...
        if existe:
            ...
            print(
                f"  ⚠️ Chatbot '{NOMBRE_CHATBOT}' ya existe (GUID: {guid}) - Estado: {estado}"
            )
