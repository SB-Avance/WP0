...
def get_token():
    ...
    try:
        ...
        if "access_token" in result:
            ...
            print(
                f"❌ Error al obtener token: {result.get('error_description', 'Unknown')}"
            )
            return None
...
def create_initial_grupos():
    ...
    for grupo in grupos_iniciales:
        ...
        if check_response.status_code == 200:
            ...
            if existing:
                print(
                    f"⏭️  Grupo '{grupo['cr321_nombre']}' ya existe (ID: {grupo['cr321_idgrupo']})"
                )
                continue
        ...
        try:
            ...
            if response.status_code == 204:
                print(
                    f"✅ Grupo '{grupo['cr321_nombre']}' creado (ID: {grupo['cr321_idgrupo']})"
                )
            else:
                print(
                    f"❌ Error creando grupo '{grupo['cr321_nombre']}': {response.status_code}"
                )
...
def create_initial_estados():
    ...
    for estado in estados_iniciales:
        ...
        if check_response.status_code == 200:
            ...
            if existing:
                print(
                    f"⏭️  Estado '{estado['cr321_nombre']}' ya existe (ID: {estado['cr321_idestado']})"
                )
                continue
        ...
        try:
            ...
            if response.status_code == 204:
                print(
                    f"✅ Estado '{estado['cr321_nombre']}' creado (ID: {estado['cr321_idestado']})"
                )
            else:
                print(
                    f"❌ Error creando estado '{estado['cr321_nombre']}': {response.status_code}"
                )
                print(f"   Detalles: {response.text}")
        except Exception as e:
