...
class SistemaMenuJerarquico:
    ...
    def _procesar_seleccion_submenu(
        ...
        self._log(
            f"✅ Selección final: {subopcion.texto} → Grupo: {subopcion.grupo_asignado}"
        )
        ...
        for idx, chatbot in enumerate(chatbots[:3], 1):  # Máximo 3 opciones principales
            ...
            if subopciones:  # Solo agregar si tiene subopciones
                opcion = OpcionPrincipal(
                    numero=orden, texto=nombre, subopciones=subopciones
                )
    ...
    def _obtener_menu_hardcoded(self) -> List[OpcionPrincipal]:
        ...
        return [
            OpcionPrincipal(
                ...
                subopciones=[
                    SubOpcion(
                        "1.1", "Soporte Técnico", "Soporte Técnico", "Solicitud Ticket"
                    ),
                    SubOpcion("1.2", "Garantías", "Garantías", "Solicitud Ticket"),
                    SubOpcion(
                        "1.3", "Consultas Generales", "Consultas", "Solicitud Ticket"
                    ),
                ],
            ),
            OpcionPrincipal(
            ...
            OpcionPrincipal(
                ...
                subopciones=[
                    ...
                    SubOpcion(
                        "3.2", "Agendar Cita", "Agendamiento", "Solicitar Atención"
                    ),
                ],
            ),
        ]
    ...
    def _es_cache_valido(self) -> bool:
        ...
        expiracion = self._cache_timestamp + timedelta(
...
def crear_sistema_menu_jerarquico(
    dataverse_url: str, token_provider_func
) -> SistemaMenuJerarquico:
    """
