...
def create_ticket():
    ...
    try:
        ...
        if response.status_code == 204:
            return (
                jsonify({"message": "Ticket creado exitosamente", "idticket": next_id}),
                201,
            )
        else:
            return (
                    ...
                    jsonify(
                        {
                            "ticket": {
                                ...
                                "estado": ticket.get("cr321_estado"),
                                "fechacreacion": ticket.get("cr321_fechacreacion"),
                                "fechaactualizacion": ticket.get(
                                    "cr321_fechaactualizacion"
                                ),
                            }
                        }
                    ),
...
def update_ticket(idticket):
    ...
    try:
        ...
        payload = {
            "cr321_fechaactualizacion": datetime.now(timezone.utc)
            .isoformat()
            .replace("+00:00", "Z")
        }
