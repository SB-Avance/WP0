...
import requests
from goot import (ACCESS_TOKEN, DATAVERSE_URL, PHONE_NUMBER_ID, get_token,
...
MENU_CACHE = {"menu": {}, "last_update": None}
...
def crear_contacto_automatico(telefono, nombre=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if contactos:
                print(
                    f"[CONTACTO] Contacto existente: {contactos[0]['cr321_contactoid']}"
                )
...
def load_menu_from_dataverse():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not chatbots:
                print(
                    "[WEBHOOK] No se encontraron chatbots activos, usando menú por defecto"
                )
                return get_default_menu()
            ...
            print(
                f"[WEBHOOK] Construyendo menú desde {len(chatbots)} chatbots activos:"
            )
            ...
            for idx, chatbot in enumerate(chatbots, 1):
                ...
                if not sub_opciones:
                    print(
                        f"  [!] ADVERTENCIA: {chatbot_name} sin sub-opciones, omitiendo"
                    )
                    continue
                ...
                if len(sub_opciones) > 10:
                    print(
                        f"  [!] ADVERTENCIA: {chatbot_name} tiene {len(sub_opciones)} sub-opciones, usando solo las primeras 10"
                    )
                ...
                try:
                    if config_json:
                        ...
                        print(
                            f"  📦 Config JSON cargada: {list(handlers_config.keys())}"
                        )
                ...
                tipo = map_nombre_to_tipo(
                    sub_opciones[0] if sub_opciones else chatbot_name
                )
                ...
                menu_opciones[str(idx)] = {
                    ...
                    "grupo_id": chatbot.get(
                        "cr321_grupoid"
                    ),  # 🆕 GUID del grupo de Dataverse
            ...
            if not menu_opciones:
                print(
                    "[WEBHOOK] No se pudieron construir opciones de menú, usando menú por defecto"
                )
                return get_default_menu()

            print(
                f"[WEBHOOK] Menú cargado exitosamente: {len(menu_opciones)} opciones principales"
            )
...
def map_nombre_to_tipo(nombre):
    ...
    if (
        "ticket" in nombre_lower
        or "soporte" in nombre_lower
        or "solicitud" in nombre_lower
    ):
        return "soporte"
        ...
        "agente" in nombre_lower
        or "atencion" in nombre_lower
        or "atención" in nombre_lower
    ):
...
def get_mensajes_for_tipo(tipo):
    ...
    if tipo == "soporte":
        ...
        base_mensajes["descripcion"] = (
            "Describa el producto, marca y modelo si lo tiene:"
        )
    elif tipo == "informacion":
        base_mensajes["respuesta"] = (
            "Gracias por contactarnos. Visite nuestro sitio web para más información."
        )
    elif tipo == "atencion_agente":
        # Solo nombre
        pass
...
def get_current_menu():
    ...
    if (
        MENU_CACHE["last_update"] is None
        or (now - MENU_CACHE["last_update"]).total_seconds() > 300
    ):
        ...
        print(
            f"[MENU] Usando cache (último update hace {int((now - MENU_CACHE['last_update']).total_seconds())}s)"
        )
...
def procesar_seleccion_elemento(
    phone, message_text, state, menu_opciones, handler_manager
):
    ...
    try:
        ...
        if 1 <= idx <= len(sub_opciones):
            ...
            print(
                f"[SUBMENU] Elemento seleccionado por índice {idx}: {elemento_seleccionado}"
            )
        else:
            return (
                f"❌ Opción inválida. Por favor elige un número entre 1 y {len(sub_opciones)} (o 0 para menú principal).\n\n"
        ...
        for sub_opcion in sub_opciones:
            if message_text.strip().lower() == sub_opcion.lower():
                ...
                print(
                    f"[SUBMENU] Elemento seleccionado por nombre: {elemento_seleccionado}"
                )
                break

        if not elemento_seleccionado:
            return (
                f"❌ Opción no encontrada. Por favor elige un número entre 1 y {len(sub_opciones)}.\n\n"
                + get_submenu_text(categoria_id)
            )
    ...
    if handler_code and handler_manager.handler_existe(handler_code):
        print(
            f"[HANDLER] Iniciando handler {handler_code} para '{elemento_seleccionado}'"
        )
...
def process_menu_response(phone, message_text):
    ...
    if phone in conversation_states:
        ...
        if state.get("modo") == "esperando_elemento":
            return procesar_seleccion_elemento(
                phone, message_text, state, menu_opciones, handler_manager
            )
        ...
        if handler_code:
            ...
            if step < len(preguntas):
                ...
                print(
                    f"[HANDLER] Respuesta guardada: {pregunta_actual[:30]}... = {message_text[:30]}..."
                )

                # Si es el último paso, ejecutar handler
                if step == len(preguntas) - 1:
                    print(
                        f"[HANDLER] Última pregunta contestada, ejecutando handler {handler_code}"
                    )
            ...
            if step < len(preguntas):
                ...
                if step == len(preguntas) - 1:
                    ...
                    ticket_data = {"tipo": opcion.get("tipo"), **state["data"]}
                    ...
                    state["step"] = step + 1
                    siguiente_pregunta = preguntas[state["step"]]
                    return opcion["mensajes"].get(
                        siguiente_pregunta, "Por favor proporcione la información:"
                    )
                    ...
                    for message in messages:
                        ...
                        if message_type == "text":
                            ...
                            try:
                                ...
                                if exito:
                                    print(
                                        f"✅ Mensaje y contacto procesados correctamente: {contacto_id}"
                                    )
                                else:
                                    print(
                                        f"⚠️ Error al procesar mensaje/contacto para {phone}"
                                    )
                                ...
                                try:
                                    ...
                                    if contacto_id:
                                        print(
                                            f"✅ Contacto procesado (fallback): {contacto_id}"
                                        )
