...
def get_whatsapp_accounts():
    ...
    try:
        ...
        if response.status_code == 200:
                ...
                accounts.append(
                    {
                        ...
                        "access_token": item.get("cr321_accesstoken", "")[:20]
            ...
            return (
                jsonify({"error": f"Error al obtener cuentas: {response.status_code}"}),
...
def get_active_accounts():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (
                jsonify({"error": f"Error: {response.status_code}"}),
                response.status_code,
            )
...
def create_whatsapp_account():
    ...
    if not nombre or not phone_number_id or not access_token:
        return (
            jsonify({"error": "Nombre, phone_number_id y access_token son requeridos"}),
            400,
        )
    ...
    try:
        ...
        if response.status_code in [200, 201, 204]:
            return (
                jsonify({"success": True, "message": "Cuenta creada exitosamente"}),
                201,
            )
        else:
            return (
...
def update_whatsapp_account(account_id):
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            return (
                jsonify(
                    {"success": True, "message": "Cuenta actualizada exitosamente"}
                ),
                200,
            )
        else:
