...
from goot import (ACCESS_TOKEN, CLIENT_ID, CLIENT_SECRET, DATAVERSE_URL,
                  ENTITY_SET, PHONE_NUMBER_ID, PHONE_NUMBER_ID0,
                  PHONE_NUMBER_ID1, TENANT_ID, VERIFY_TOKEN, clear_screen)
...
MESSAGE_TYPES = {"text": 462410000, "image": 462410001, "audio": 462410002}

# Mapeo de dirección
MESSAGE_DIRECTION = {"incoming": 462410000, "outgoing": 462410001}
...
def save_bot_response(
    phone_number, fromname, bot_message, timestamp, message_id=None, group=None
):
...
def save_to_dataverse(
    message_id, fromphone, timestamp, message_type, body, fromname, group=None
):
    ...
    try:
        response = requests.post(url, headers=headers, json=payload)
        print(
            f"[SAVE_DATAVERSE] {response.status_code} - De: {fromname} ({fromphone}) - Grupo: {group}"
        )
    except Exception as e:
...
def send_reply(phonenumber, text, timestamp, fromname="", group=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            save_bot_response(
                phonenumber, fromname, message, timestamp, message_id, group
            )
    except Exception as e:
        ...
        for entry in data.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for message in messages:
                    ...
                    print(
                        f"[WEBHOOK] Mensaje recibido de {fromphone}: '{body}' -> Grupo: {group}"
                    )
...
def get_conversations():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify(
                {"success": True, "conversations": conv_list, "groups": list(groups)}
            )
        else:
...
def send_manual_message():
    ...
    print(
        f"[SEND_MESSAGE] Recibiendo solicitud: phone={phone}, message={message}, group={group}"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"[SEND_MESSAGE] Guardando en Dataverse: message_id={message_id}, grupo={group}"
            )
            save_result = save_bot_response(
                clean_phone, "", message, timestamp, message_id, group
            )
            print(f"[SEND_MESSAGE] Resultado de guardar en Dataverse: {save_result}")

            return jsonify({"success": True, "message_id": message_id})
        else:
            return jsonify({"success": False, "error": response.json()}), 500
...
if __name__ == "__main__":
    ...
    is_production = (
        os.environ.get("WEBSITE_INSTANCE_ID") is not None
    )  # Variable de Azure App Services
    debug_mode = False  # Desactivar debug temporalmente para Windows

    print(
        f"[STARTUP] Modo: {'PRODUCCION (Azure)' if is_production else 'DESARROLLO (Local)'}"
    )
