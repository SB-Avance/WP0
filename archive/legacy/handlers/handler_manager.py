...
class HandlerManager:
    ...
    def _load_config(self):
        ...
        try:
            ...
            print(
                f"[HANDLER_MANAGER] Configuración cargada: {len(self.handlers_config)} handlers"
            )

            # Mostrar handlers activos
            activos = [
                codigo
                for codigo, cfg in self.handlers_config.items()
            ...
            print(
                f"[HANDLER_MANAGER] ⚠️ No se encontró config_handlers.json en {self.config_path}"
            )
            self.handlers_config = {}
    ...
    def get_handler(self, codigo: str) -> Optional[BaseHandler]:
        ...
        try:
            ...
            print(
                f"[HANDLER_MANAGER] ✓ Handler {codigo} cargado: {config.get('nombre')}"
            )
    ...
    def ejecutar_handler(
        ...
        try:
            ...
            mensaje_respuesta = handler.ejecutar_accion_final(
                from_user=from_user, respuestas=respuestas, grupo_id=grupo_id_final
            )
